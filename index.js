...
// ==================== WIADOMOŚĆ CO 2 LEGIT CHECKI ====================
let legitCheckCounter = 0;

client.on(Events.MessageCreate, async (message) => {
  if (message.channel.id !== '1286040567221846121') return;
  if (message.author.bot) return;

  if (message.content.toLowerCase().includes('+rep')) {
    legitCheckCounter++;

    if (legitCheckCounter % 2 === 0) {
      const instrukcjaEmbed = new EmbedBuilder()
        .setColor('#00ff00')
        .setTitle('✅ WYMIEŃ HAJS × JAK NAPISAĆ LEGIT CHECKA?')
        .setDescription(
          '**WZÓR:**\n' +
          '`+rep @centraldm Exchanged [Z CZEGO] to [NA CO] [ILOŚĆ] PLN`\n\n' +
          '**PRZYKŁAD:**\n' +
          '`+rep @centraldm Exchanged BLIK to LTC 100 PLN`'
        );

      await message.channel.send({ embeds: [instrukcjaEmbed] });
    }
  }
});
